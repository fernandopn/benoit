...
- For `-provider openrouter`, the model's context window is fetched at startup from OpenRouter's `/api/v1/models` (`context_length`), so context-usage percentages show in the UI like the OpenAI provider — with no hardcoded sizes. If the lookup fails, context usage is reported without a percentage.
